...
//        printf("%ld: %f, %f avg, %e rate, %lf seconds, %d images\n", get_current_batch(net), loss, avg_loss, get_current_rate(net), what_time_is_it_now()-time, i*imgs);
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile, float nms)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.list");
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, float nms, char *outfile, int fullscreen)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
...
void testbatch_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, float nms, int fullscreen)
{
	printf("testbatch_detector()\n");
    printf("filename %s\n", filename);
...
void run_detector(int argc, char **argv)
{
    ...
    float nms = find_float_arg(argc, argv, "-nms", 0.45);
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, nms, outfile, fullscreen);
    else if(0==strcmp(argv[2], "testbatch")) testbatch_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, nms, fullscreen);
    else if(0==strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear);
    else if(0==strcmp(argv[2], "valid")) validate_detector(datacfg, cfg, weights, outfile, nms);
